...
from conversion_utils import henon_to_binary, kaplan_yorke_to_binary, tinkerbell_to_integer
def generate_key_sequence(key_float):
    ...
    key_bin = []
    for i in range (0, 6):
        key_bin.append(utilities.hex_to_binary(key_sha[i]))
    
    # B to N
    
    input = []
    for i in range (0, 6):
        input.append((utilities.binary_to_int(key_bin[i][:50]))*(10**-16))
    ...
    str3 = utilities.xor_binary_strings(str1, str2)

    # reshape to matrix

    str3_mat = utilities.string_to_16x8_matrix(str3)
    ...
    final_mat = utilities.rearrange_matrix_rows(str3_mat, Tb)
    # revert matrix reshape
    km = ""
    for i in range (0, len(final_mat)):
        for j in range (0, len(final_mat[0])):
            km+=(final_mat[i][j])
    final_km = utilities.binary_to_int_list(km)
