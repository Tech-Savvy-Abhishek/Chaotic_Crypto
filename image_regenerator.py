import cv2
import numpy as np
from .image_processor import convert_bgr_to_3channel , convert_to_1d_BGR 
from .image_encryptor import preprocess_key
...
def decrypt_to_XOR(img_path , key):
    ...
    NBm, NGm, NRm = XOR_array(channel_array[0], channel_array[1], channel_array[2], key)
...
def get_original_img(Rm , Gm , Bm , original_shapes):
    R_channel = [np.array(Rm[i*original_shapes[1]: (i+1)*original_shapes[1]]) for i in range(original_shapes[0])]
    G_channel = [np.array(Gm[i*original_shapes[1]: (i+1)*original_shapes[1]]) for i in range(original_shapes[0])]
    B_channel = [np.array(Bm[i*original_shapes[1]: (i+1)*original_shapes[1]]) for i in range(original_shapes[0])]
